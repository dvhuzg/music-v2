...
let volumn_slider = document.querySelector(".volumn_slider");
let curr_time = document.querySelector(".current-time");
let total_duration = document.querySelector(".total-duration");
let wave = document.getElementById("wave");
let no_wave = document.getElementById("no_wave");
...
function loadTrack(track_index) {
  ...
  curr_track.addEventListener("end", nextTrack);
  random_bg_color();
}
...
curr_track.addEventListener("ended", function () {
  if (track_index == music_list.length && isRepeat == true) {
    track_index = 0;
    loadTrack(track_index);
    playTrack();
...
function nextTrack() {
  if (track_index < music_list.length - 1 && isRandom == false) {
    ...
    loadTrack(track_index);
    playTrack();
  } else if (track_index >= music_list.length - 1 && isRepeat == true) {
    track_index = 0;
    loadTrack(track_index);
    playTrack();
  } else if (track_index >= music_list.length - 1 && isRepeat == false) {
    let current_index = track_index;
    loadTrack(current_index);
    pauseTrack();
  }
...
function setVolumn() {
  curr_track.volume = volumn_slider.value / 100;
}
function setUpdate() {
  let seekPosition = 0;
